...
class ComputeAllAtomCoords(nn.Module):
    ...
    def forward(self, seq, xyz, alphas, non_ideal=False, use_H=True):
        ...
        # omega
        RTF1 = torch.einsum(
            "brij,brjk,brkl->bril", RTF0, self.RTs_in_base_frame[seq, 0, :], make_rotX(alphas[:, :, 0, :])
        )

        # phi
        RTF2 = torch.einsum(
            "brij,brjk,brkl->bril", RTF0, self.RTs_in_base_frame[seq, 1, :], make_rotX(alphas[:, :, 1, :])
        )

        # psi
        RTF3 = torch.einsum(
            "brij,brjk,brkl->bril", RTF0, self.RTs_in_base_frame[seq, 2, :], make_rotX(alphas[:, :, 2, :])
        )
        ...
        # chi2
        RTF5 = torch.einsum(
            "brij,brjk,brkl->bril", RTF4, self.RTs_in_base_frame[seq, 4, :], make_rotX(alphas[:, :, 4, :])
        )

        # chi3
        RTF6 = torch.einsum(
            "brij,brjk,brkl->bril", RTF5, self.RTs_in_base_frame[seq, 5, :], make_rotX(alphas[:, :, 5, :])
        )

        # chi4
        RTF7 = torch.einsum(
            "brij,brjk,brkl->bril", RTF6, self.RTs_in_base_frame[seq, 6, :], make_rotX(alphas[:, :, 6, :])
        )
        ...
        xyzs = torch.einsum(
            "brtij,brtj->brti",
            RTframes.gather(2, self.base_indices[seq][..., None, None].repeat(1, 1, 1, 4, 4)),
            basexyzs,
        )
...
class XYZConverter(nn.Module):
    ...
    def get_torsions(self, xyz_in, seq, mask_in=None):
        ...
        torsions[:, :, 7, :] = torch.stack(
            (torch.sum(t * t0, dim=-1), t[..., 0] * t0[..., 1] - t[..., 1] * t0[..., 0]), dim=-1
        )
        ...
        NCp = xyz[:, :, 2, :3] - xyz[:, :, 0, :3]
        NCpp = (
            NCp - torch.sum(NCp * NCCA, dim=-1, keepdim=True) / torch.sum(NCCA * NCCA, dim=-1, keepdim=True) * NCCA
        )
